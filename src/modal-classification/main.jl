...
function _convert(
    ...
    if node.is_leaf
        ...
        return DTNode{S, T}(node.feature, node.threshold, left, right)
    end
end
...
function prune_tree(tree::DTNode{S, T}, purity_thresh=1.0) where {S, T}
    if purity_thresh >= 1.0
        return tree
    end
    function _prune_run(tree::DTNode{S, T}, purity_thresh::Real) where {S, T}
        N = length(tree)
        if N == 1        ## a DTLeaf
            return tree
        elseif N == 2    ## a stump
            ...
            return DTNode{S, T}(tree.featid, tree.featval,
...
function apply_tree(tree::DTNode{S, T}, features::AbstractVector{S}) where {S, T}
    if tree.featid == 0
...
function apply_tree_proba(tree::DTNode{S, T}, features::AbstractVector{S}, labels) where {S, T}
