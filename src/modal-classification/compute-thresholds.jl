		...
		@inbounds for feature in 1:n_vars
			...
			for i in 1:n_instances
				...
				for w in ModalLogic.enumAcc(X.ontology.worldType[], ModalLogic.RelationAll, channel)
					# opGeqMaxThresh, opLesMinThresh = ModalLogic.WMin(w, channel), ModalLogic.WMax(w, channel)
					opGeqMaxThresh, opLesMinThresh = ModalLogic.WExtrema(w, channel)
					Sogliole[w.x.x, w.x.y, w.y.x, w.y.y, i,relationId_id,feature] = (opGeqMaxThresh, opLesMinThresh)
				end
				...
				for relation_id in relation_ids
					...
					for w in ModalLogic.enumAcc(X.ontology.worldType[], ModalLogic.RelationAll, channel)
						# @info "World " w 
						opGeqMaxThresh, opLesMinThresh = ModalLogic.WExtrema(ModalLogic.enumAccRepr(w, relation, channel), channel)
						# opGeqMaxThresh, opLesMinThresh = typemin(T), typemax(T)
						# inverted,representatives = ModalLogic.enumAccRepr(w, relation, channel)
						# for v in representatives
						# # for v in ModalLogic.enumAcc([w], relation, channel)
						# 	# (v_opGeqMaxThresh, v_opLesMinThresh) = ModalLogic.WExtrema(v, channel)
						...
						# @assert (opGeqMaxThresh, opLesMinThresh) == ModalLogic.WExtrema(ModalLogic.enumAccRepr(w, relation, channel), channel) "Wextrema different $((opGeqMaxThresh, opLesMinThresh)) $(ModalLogic.WExtrema(w, channel))"

						@info "World " relation_id relation w opGeqMaxThresh opLesMinThresh
						Sogl[w.x.x, w.x.y, w.y.x, w.y.y] = (opGeqMaxThresh, opLesMinThresh)
					end
				end # relation
