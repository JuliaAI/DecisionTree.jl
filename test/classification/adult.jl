...
p1 = permutation_importance(model, labels, features, (model, y, X)->accuracy(y, apply_forest(model, X)), rng = 1).mean
...
p2 = permutation_importance(model, labels, features, (model, y, X)->accuracy(y, apply_forest(model, X)), rng = 1).mean
...
pruning_purity = 0.9
nfolds = 3
accuracy1 = nfoldCV_tree(labels, features, nfolds, pruning_purity; verbose=false);
...
accuracy1 = nfoldCV_forest(labels, features, n_folds, n_subfeatures, n_trees, partial_sampling; verbose=false)
@test mean(accuracy1) > 0.8

println("\n##### nfoldCV Classification Adaboosted Stumps #####")
n_iterations = 15
n_folds = 3
accuracy1 = nfoldCV_stumps(labels, features, n_folds, n_iterations; verbose=false);
@test mean(accuracy1) > 0.8
