...
nfolds = 3
accuracy1 = nfoldCV_tree(labels, features, nfolds)
...
n_folds = 3
partial_sampling = 0.5
accuracy1 = nfoldCV_forest(labels, features, nfolds, n_subfeatures, n_trees, partial_sampling)
@test mean(accuracy1) > 0.9
...
n_iterations = 15
nfolds = 3
accuracy1 = nfoldCV_stumps(labels, features, nfolds, n_iterations)
@test mean(accuracy1) > 0.85
